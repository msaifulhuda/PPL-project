<x-app-guru-layout>
    <div class="px-3 py-5 mx-4 my-6 bg-white rounded-lg shadow xl:p-6">
        ...
        {{-- Main Content --}}
        <div class="flex px-3 md:flex-row">
            {{-- Materi Pelajaran --}}
            <div class="flex flex-col gap-4 mt-6 ">
                ...
                <div class="grid gap-4 sm:grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
                    ...
                    @if ($kelas_mata_pelajaran->count() > 0)
                        @foreach ($kelas_mata_pelajaran as $kelas)
                            <a href="{{ route('guru.dashboard.lms.materi.create', $kelas->id_kelas_mata_pelajaran) }}" class="block">
                                <div class="flex items-center p-4 transition duration-100 border-2 border-gray-300 rounded-2xl hover:bg-gray-100">
                                    ...
                                    <div>
                                        <h4 class="text-lg font-semibold text-gray-800">{{$kelas->mata_pelajaran->nama_matpel}} {{ $kelas->kelas->nama_kelas }}</h4>
                                        </h4>
                                        <p class="text-sm text-gray-500">{{$kelas->hari->nama_hari}}, {{$kelas->waktu_mulai}} - {{$kelas->waktu_selesai}} WIB</p>
                                    </div>
                                </div>
                            </a>
                        @endforeach
