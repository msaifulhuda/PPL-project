<x-siswa-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6 relative">
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">Dashboard Ekstrakurikuler <b>{{ $ekstra->ekstrakurikuler->nama_ekstrakurikuler }}</b></h2>
                <!-- Tombol untuk membuka/tutup registrasi -->
                <form id="statusForm" method="post" action="{{ route('dashboard.status') }}">
                    @csrf
                    @method('PUT')
                    <input type="hidden" name="status" id="statusInput" value="{{ $rilStatus == 'tidak buka' ? 'buka' : 'tidak buka' }}">
                    <button 
                        type="button"
                        id="toggleButton" 
    ...
    <!-- Modal -->
    <div id="confirmationModal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-gray-900 bg-opacity-50">
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                ...
                <div class="flex justify-end mb-4">
                    <button onclick="toggleCreateModal()" class="px-4 py-2 bg-blue-500 text-white rounded-lg shadow hover:bg-blue-700">
                        Posting
                    </button>
                </div>
                    ...
                    <div class="bg-white rounded-lg shadow-lg p-6 w-1/2">
                        ...
                        <form action="{{ route('dashboard.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                            ...
                            <div>
                                ...
                                <trix-editor input="deskripsi" class="border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 rounded-md"></trix-editor>
                            </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    @forelse($postings as $index => $posting)
                        <div class="border p-4 rounded-lg shadow-sm bg-white">
                            <!-- Judul Postingan -->
                            <h4 class="text-lg font-semibold text-gray-800 mb-2">{{ Str::limit($posting->judul,50 )}}</h4>
                            ...
                            <div class="flex justify-between items-center">
                                <!-- Tombol Edit -->
                                <button 
                                    data-modal-target="edit-{{ $index }}" data-modal-toggle="edit-{{ $index }}" 
                        ...
                        <div class="relative bg-white rounded-lg shadow-lg w-full max-w-2xl">
                            ...
                            <div class="flex items-center justify-between p-4 border-b rounded-t bg-indigo-600">
                                ...
                                <button type="button" class="text-gray-200 bg-transparent hover:bg-indigo-700 hover:text-white rounded-lg text-sm w-8 h-8 flex items-center justify-center"
                            ...
                            <div class="p-6 space-y-4">
                                <form action="{{ route('dashboard.update', ['id_posting' => $posting->id_posting]) }}" id="editForm" method="POST" enctype="multipart/form-data">
                                    @csrf
                                    @method('PUT')
                                    <!-- Judul -->
                                    <div>
                                        <label for="editJudul" class="block text-sm font-medium text-gray-700">Judul</label>
                                        <input type="text" name="judul" id="editJudul" value="{{ $posting->judul }}"  required
                                    ...
                                    <div>
                                        ...
                                        <input type="file" name="gambar" id="editGambar" accept="image/*"
                                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
                                        <img id="currentImage" class="mt-4 max-w-full rounded-lg border border-gray-200 shadow" />
                                    </div>

                                    <!-- Deskripsi -->
                                    <div>
                                        <label for="editDeskripsi" class="block text-sm font-medium text-gray-700">Deskripsi</label>
                                        <input id="editDeskripsi" type="hidden" name="deskripsi" value="{{ $posting->deskripsi }}">
